...
stock radioStreamForPlayer(playerid)
{
    ...
    if(GetPVarInt(playerid, "iPlayerRadioDisplayMsg") == 0)
    {
        ...
        ShowBoxForPlayer(playerid, "Tuned in to ~y~LVP RADIO~w~. Use the ~p~/audiomsg~w~ SA-MP command to hide radio chat messages. Use ~p~/radio~w~ to toggle radio options.");
    }
