...
class BankCommands {
    ...
    /**
     * Administrators have the ability to change the bank account value of each player on the server
     * by typing "/p [player] bank". This command also allows them to get the balance of the player.
     *
     * @param playerId Id of the player or administrator who send the command.
     * @param subjectId Id of the player who's subject of the command.
     * @param balance Balance as set by the crew member. Optional.
     * @command /p [player] bank [balance]?
     */
    @switch(PlayerCommand, "bank")
    public onPlayerBankCommand(playerId, subjectId, params[]) {
        if (Player(playerId)->isAdministrator() == false)
            return 0;

        new message[128];
        if (Command->parameterCount(params) >= 1) {
            new forcedBalance = Command->integerParameter(params, 0);
            if (forcedBalance < 0) {
                SendClientMessage(playerId, Color::Error, "The new balance needs to be a positive number.");
                return 1;
            }

            if (forcedBalance > BankAccount(subjectId)->maximumBalance()) {
                format(message, sizeof(message), "%s's bank account type allows for a maximum of {40CCFF}$%s{FFFFFF} to be stored.",
                    Player(subjectId)->nicknameString(), formatPrice(BankAccount(subjectId)->maximumBalance()));
                SendClientMessage(playerId, Color::Error, message);
                return 1;
            }

            // We can now safely update the balance as it's stored in this player's bank account. Do
            // inform the player of this action as well, however, as we do with other admins.
            BankAccount(subjectId)->setBalance(forcedBalance);

            format(message, sizeof(message), "** %s (Id:%d) has changed your bank account balance to {40CCFF}$%s{FFFFFF}.",
                Player(playerId)->nicknameString(), playerId, formatPrice(forcedBalance));
            SendClientMessage(subjectId, Color::Information, message);

            format(message, sizeof(message), "You have changed the bank account balance of {40CCFF}%s{FFFFFF} to {40CCFF}%s{FFFFFF}.",
                Player(subjectId)->nicknameString(), formatPrice(forcedBalance));
            SendClientMessage(playerId, Color::Information, message);

            format(message, sizeof(message), "%s (Id:%d) changed the bank account balance of %s (Id:%d) to $%s.",
                Player(playerId)->nicknameString(), playerId,
                Player(subjectId)->nicknameString(), subjectId,
                formatPrice(forcedBalance));
            Admin(playerId, message);

            // And we're all done here. Bail out.
            return 1;
        }

        // No parameters were passed, give the administrator information about this player's balance.
        format(message, sizeof(message), "The balance of %s (Id:%d) currently is {40CCFF}$%s{FFFFFF}.",
            Player(subjectId)->nicknameString(), subjectId, formatPrice(BankAccount(subjectId)->balance()));
        SendClientMessage(playerId, Color::Information, message);
        SendClientMessage(playerId, Color::Information, "  Change this balance by using \"/p [player] bank [balance]\".");

        format(message, sizeof(message), "The player currently has a {40CCFF}%s{FFFFFF} account.",
            BankAccount(subjectId)->type() == NormalBankAccount ? "normal" : "Premier");
        SendClientMessage(playerId, Color::Information, message);
