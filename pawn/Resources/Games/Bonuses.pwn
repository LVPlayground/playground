    ...
    switch(Bonus)
    {
        case 0:
        {
            format(szMessageToAll, 128, "* The bonus time started: everyone who wins a minigame within the next %d minutes gets $%s bonus cash.",
                BONUS_TIME/60, formatPrice(GetEconomyValue(BonusMinigame)));
        ...
        case 1:
        {
            format(szMessageToAll, 128, "* The bonus time started: everyone who exports a vehicle within the next %d minutes gets $%s bonus cash.",
                BONUS_TIME/60, formatPrice(GetEconomyValue(BonusExport)));
            SendClientMessageToAll(COLOR_PINK, szMessageToAll);
            for(new i; i<sizeof(BonusTime); i++)
            {
                BonusTime[i] = false;
        ...
        case 2:
        {
            format(szMessageToAll, 128, "* The bonus time started: everyone who kills somebody within the next %d minutes gets $%s bonus cash.",
                BONUS_TIME/60, formatPrice(GetEconomyValue(BonusKill)));
            SendClientMessageToAll(COLOR_PINK, szMessageToAll);
            for(new i; i<sizeof(BonusTime); i++)
            {
                BonusTime[i] = false;
...
BonusTime__CheckPlayer(playerid, BonusCase)
{
    ...
    if(BonusCase == BONUS_MINIGAMES)
    {
        if(BonusTime[BONUS_MINIGAMES] == true)
        {
            ...
            format(szMessageToPlayer, 128, "* You earned $%s by doing this.", formatPrice(GetEconomyValue(BonusMinigame)));
            SendClientMessage(playerid, COLOR_PINK, szMessageToPlayer);
            GiveRegulatedMoney(playerid, BonusMinigame);
        }
    }
    ...
    if(BonusCase == BONUS_EXPORT)
    {
        if(BonusTime[BONUS_EXPORT] == true)
        {
            ...
            format(szMessageToPlayer, 128, "* You earned $%s by doing this.", formatPrice(GetEconomyValue(BonusExport)));
            SendClientMessage(playerid, COLOR_PINK, szMessageToPlayer);
            GiveRegulatedMoney(playerid, BonusExport);
        }
    }
    ...
    if(BonusCase == BONUS_KILL)
    {
        if(BonusTime[BONUS_KILL] == true)
        {
            ...
            format(szMessageToPlayer, 128, "* You earned $%s by doing this.", formatPrice(GetEconomyValue(BonusKill)));
            SendClientMessage(playerid, COLOR_PINK, szMessageToPlayer);
            GiveRegulatedMoney(playerid, BonusKill);
        }
    }
